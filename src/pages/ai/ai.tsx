...
export default function AI({ owner = "facebook", repo = "react" }) {
  ...
  useEffect(() => {
    const fetchAccessToken = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/auth/access-token`, {
      ...
      <SidebarMenuItem>
        <Collapsible className="group/collapsible" open={isOpen}>
          <CollapsibleTrigger asChild>
            <SidebarMenuButton onClick={handleToggle} className="hover:bg-black/10 truncate">
                ...
                children.map((childItem) =>
                  childItem.type === "dir" ? (
                    ...
                    <SidebarMenuItem key={childItem.path}>
                      <SidebarMenuButton
                        onClick={async () => {
                          const content = await fetchFileContent(childItem.path);
      ...
      <SidebarProvider className="w-dvw">
        <Sidebar className="flex h-dvh">
          <SidebarHeader className="px-4 h-16 border-b bg-white">
            <Link to="/" className="text-[var(--ten)] font-extrabold text-3xl h-full flex items-center gap-2">
              <img src="/assets/logo.svg" className="w-full max-w-32 h-16" alt="logo" />
              <Badge className="text-lg min-w-11 text-center font-bold bg-primary" variant="outline">
                AI
              </Badge>
            </Link>
          </SidebarHeader>
        ...
        <main className="w-dvw">
          ...
          {selectedFileContent ? (
            <MonacoEditor
              ...
              className="w-dvw h-[calc(100dvh_-_4rem)]"
            />
          ) : (
            <p className="h-[calc(100dvh_-_4rem)] text-center flex items-center justify-center">
