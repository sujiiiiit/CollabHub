...
interface HeaderProps {
  onHeightChange: (height: number) => void; // Define the prop
}
...
const Header: React.FC<HeaderProps> = ({ onHeightChange }) => {
  ...
  return (
    <header id="header" className=" z-50 bg-white h-16 border-b" ref={headerRef}>
      <div className="header max-w-6xl mx-auto flex justify-between h-16 px-4 items-center">
        ...
        <div className="flex gap-2 items-center justify-between h-full">
          ...
          {user ? (
            ...
            <Button onClick={handleLogin} className="rounded-full gap-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
                height="20"
